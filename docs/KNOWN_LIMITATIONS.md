...
- O MVP suporta somente Google Chrome 116 ou superior. A versão `1.0.0` está preparada para revisão, mas ainda não foi publicada na Chrome Web Store.
...
- A Fase 5 foi aprovada em uma rodada manual, sem bugs relatados. O ZIP público `1.0.0` ainda precisa ser instalado a partir do pacote final e submetido à revisão da Chrome Web Store.
